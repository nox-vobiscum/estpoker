/* ===================== Estimation Poker — room.js ==========================
   - Liest Name/Room aus <body data-*> mit URL/DOM-Fallback
   - Initialisiert zuverlässig (egal ob DOM bereits "interactive/complete")
   - Stabiler WebSocket mit Reconnect + Leave-Beacon
   ======================================================================== */
...
(function boot(){
  ...
  const nameFromDom = document.getElementById('youName')?.textContent?.trim();
  const roomFromDom = document.getElementById('roomCodeVal')?.textContent?.trim();

  window.__ROOM = {
    participantName: ds.participant || qs.get('participantName') || nameFromDom || 'Guest',
    roomCode:        ds.room        || qs.get('roomCode')        || roomFromDom || ''
  };
  try { console.log('[ROOM] boot', window.__ROOM); } catch {}
})();

// ----- State -----
let { participantName, roomCode } = window.__ROOM;
...
const TXT_AVG='Avg:', TXT_CONS='Consensus:', TXT_MEDIAN='Median:', TXT_RANGE='Range:',
      ...
      TXT_AR_ONLY_HOST='Only the host can change this setting.',
      TXT_MAKE_HOST='Make host', TXT_KICK='Kick', TXT_KICKED='The host has closed the room for you.',
...
// ----- Deck / UI helpers (unverändert zu deiner Version) -----
function formatDeckForTooltip(cards){
  ...
  const shown    = core.slice(0, 12);
  const more     = core.length > 12 ? '…' : '';
...
function applyTopicUI(){
  ...
  const eb=document.getElementById('topicEditBtn');
  const cb=document.getElementById('topicClearBtn');
...
function connectWS(){
  try { console.log('[ROOM] WS →', baseSocketUrl); } catch {}
  socket = new WebSocket(baseSocketUrl);
  socket.onopen    = () => { try { console.log('[ROOM] WS open'); } catch {} reconnectAttempts=0; };
  socket.onmessage = handleMessage;
  socket.onerror   = (e) => { try { console.warn('[ROOM] WS error', e); } catch {} };
  socket.onclose   = (e) => {
    try { console.warn('[ROOM] WS closed', e.code, e.reason||''); } catch {}
...
function handleMessage(event){
  ...
  if (data.type === 'voteUpdate') {
    ...
    showToast(newHost === participantName ? 'Host changed. You are now host!' : ('Host changed. {new} is now host.').replace('{new}', newHost));
...
// Init sicher ausführen (egal in welchem Zustand)
...
// ----- Participants / Stats / Toast (wie gehabt) -----
function transferHost(name){ if(!isHost||!name||name===participantName) return; socket?.send('transferHost:'+name); }
function kick(name){ if(!isHost||!name||name===participantName) return; socket?.send('kick:'+name); }
...
function showResults(participants, avg){
  ...
  const cons=hasConsensus(participants); setConsensusUI(cons);
  if(!cons){ const stats=(function(){
    ...
    updateStatsUI(stats); markOutliers(stats?stats.outlierNums:null);
  } else {
    updateStatsUI(null); markOutliers(null);
  }
}
